...
function RestaurantSignup() {
    ...
    const register = async () => {
        try {
            ...
            let dataRef = doc(db, 'Restaurants', user.uid)
    ...
    function Login() {
        signInWithEmailAndPassword(auth, Loginemail, Loginpassword)
            .then(async(userCredential) => {
                ...
                alert("loginSucess")
               await dispatch({ type: "AUTH_USER", payload: user });
                console.log(state.authUser  , "auth return");

                localStorage.setItem("isRest",1)
            ...
            <section  className="Ssection" >
                <div className={"container"}>
                    <div className={"user signinBx"}>
                        ...
                        <div className={"formBx"}>
                            <div>
                                ...
                                <br />
                                <br />
                                <button onClick={() => { history.push("/signup") }} className={"button-two"} > <span style={{ color: "black", fontStyle: "normal", fontSize: "20px" }}>Login as Customer.?</span> </button>
                            </div>
                        </div>
                    </div>
                    <div className={"user signupBx"}>
                        <div className={"formBx"}>
                            <div>
                                <h2 className={"Formh2"} > Register Restaurant</h2>
                                ...
                                <button onClick={() => { history.push("/signup") }} className={"button-two"} > <span style={{ color: "black", fontStyle: "normal", fontSize: "20px" }}> Register Customer.?</span> </button>
                                </p>
                            </div>
                        </div>
